...
\title[Secure CPS]{Replay Attack Detection in Cyber Physical Systems}
\author[Yilin Mo]{莫一林}
\institute[Tsinghua]{
  Department of Automation, Tsinghua University
}
\date[Feb 19th, 2019]{Feb 19th, 2019\\ 
  \small Joint Work with Hanxiao Liu, Jiaqi Yan, Lihua Xie and Karl H. Johansson}
...
\section{Replay Attack and Physical Watermarking}
...
\begin{frame}{Convergence of the Watermarking Signal}
  \begin{theorem}
    Assume that $0 < \beta <1$, then we have $\hat U_k$ converges to the optimal $U$ a.s. Furthermore, for any $\epsilon > 0$, we have
    \begin{align*}
      \hat U_k - U \sim O(k^{-\gamma+\epsilon}),a.s.
    \end{align*}
where $\gamma = \max((1-\beta)/2,\beta)$.
  \end{theorem}
  \begin{enumerate}
  \item To achieve faster convergence, we need to maximize $\gamma$.
  \item The $(1-\beta)/2$ term is due to the estimation error. 
  \item If we choose $\beta \rightarrow 0^+$ (non-decaying explortation), then $(1-\beta)/2\approx 0.5$ (central limiting theorem, law of iterated logarithm).
  \item The $\beta$ term is due to the exploration term $k^{-\beta}I$.
  \item $\gamma$ is maximized when $\beta = 1/3$.
  \end{enumerate}
\end{frame}
