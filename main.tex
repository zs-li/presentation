...
\begin{frame}{Secure Dynamic State Estimation}
  \begin{itemize}
    \item Consider the following dynamic system
      \begin{align}
	x(k+1) = A x(k) + w(k),\, y(k) = C x(k) + v(k) + a(k).
      \end{align}
    \item $a(k)$ is a $c$-sparse attack signal, the support of which is unknown
    \item Consider a steady-state Kalman filter:
      \begin{align}
	\hat x(k+1) = A \hat x(k) + K(y(k+1)-CA\hat x(k)),
      \end{align}
      \begin{center}
...
\begin{frame}{Dynamic State Estimate: A Moving Horizon Approach}
  \begin{itemize}
    \item  {\bf Fundamental limit}: If the system becomes undetectable after removing $2c$ sensors, then there exists an attack on $c$ sensors, such that NO estimator can have bounded estimation error[Nakahira2018].
    \item  In order to convert the dynamic estimation problem into a static one, we can use a moving horizon approach[Fawzi2014][Shoukry2017]:
  \begin{center}
    \begin{tikzpicture}[x=2cm, y=0.75cm]
      \node at (-0.7,0) {State Est:};
      \node [blue!70] at (-0.7,1) {Sensor 2:};
...
\item However, the historical data are discarded and the estimation performance may be poor when the system is operating normally.
  \end{itemize}
\end{frame}
